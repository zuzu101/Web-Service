    ...
    <div class="row mb-2">
        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">Kelola Keunggulan</li>
            </ol>
        </div>
    </div>
</header>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Kelola Keunggulan</h3>
                    <div class="card-tools">
                        <a href="{{ route('admin.cms.advantage-section.index') }}" class="btn btn-secondary btn-sm me-2">
                            <i class="fas fa-cog"></i> Pengaturan Section
                        </a>
                        <a href="{{ route('admin.cms.advantage.reorderPage') }}" class="btn btn-info btn-sm me-2">
                            <i class="fas fa-sort"></i> Atur Urutan
                        </a>
                        <a href="{{ route('admin.cms.advantage.create') }}" class="btn btn-primary btn-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="advantageTable" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th width="5%">No</th>
                                    <th width="15%">Icon</th>
...
$(function () {
    ...
    $('#advantageTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route('admin.cms.advantage.index') }}',
        columns: [
    ...
    $('#quickEditSectionForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route('admin.cms.advantage-section.updateTitle') }}',
            type: 'POST',
            data: {
                '_token': '{{ csrf_token() }}',
                'title': title
...
function loadSectionTitle() {
    $.ajax({
        url: '{{ route('admin.cms.advantage-section.getTitle') }}',
        type: 'GET',
        success: function(response) {
...
function deleteAdvantage(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route('admin.cms.advantage.destroy', ':id') }}'.replace(':id', id),
                type: 'DELETE',
                data: {
                    '_token': '{{ csrf_token() }}'
                },
                success: function(response) {
                    if (response.success) {
                        ...
                        $('#advantageTable').DataTable().ajax.reload();
                    } else {
                        Swal.fire({
                            title: 'Error!',
                            text: response.message,
...
function toggleStatus(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route('admin.cms.advantage.toggleStatus', ':id') }}'.replace(':id', id),
                type: 'POST',
                data: {
                    '_token': '{{ csrf_token() }}'
                },
